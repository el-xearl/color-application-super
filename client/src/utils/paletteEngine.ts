...
export function generatePalette(preset: StylePreset, variationSeed: number) {
  const base = preset.baseColors.slice(0, 5);

  const hueShift = (variationSeed % 12) - 6;
  const saturationShift = ((variationSeed % 5) - 2) * 0.18;
  const brightnessShift = ((variationSeed % 7) - 3) * 0.08;
  ...
  const adjusted = scale.map((color, index) => {
    ...
    if (preset.intensity === "Extreme") {
      c = c.saturate(1 + saturationShift).darken(index < 2 ? 0.25 : 0);
    }

    if (preset.intensity === "Soft") {
      c = c.desaturate(0.5).brighten(0.25 + brightnessShift);
    }
